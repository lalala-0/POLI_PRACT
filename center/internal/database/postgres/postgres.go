...
func EnsurePostgresStructure() error {
	...
	if err := verifyTableStructure("hosts", []ColumnDefinition{
		...
		{Name: "hostname", Type: "character varying", NotNull: true},
		{Name: "ip_address", Type: "character varying", NotNull: true},
		{Name: "agent_port", Type: "integer", Default: "8081"},
		{Name: "priority", Type: "integer", Default: "0"},
		{Name: "is_master", Type: "boolean", Default: "false"},
		{Name: "status", Type: "character varying", Default: "unknown'::character varying"},
	...
	if err := verifyTableStructure("host_processes", []ColumnDefinition{
		...
		{Name: "process_name", Type: "character varying", NotNull: true},
	}); err != nil {
		return err
	}
	...
	if err := verifyTableStructure("host_containers", []ColumnDefinition{
		{Name: "id", Type: "integer", NotNull: true, PrimaryKey: true},
		{Name: "host_id", Type: "integer", NotNull: true},
		{Name: "container_name", Type: "character varying", NotNull: true},
	}); err != nil {
		return err
	}
	...
	if err := verifyTableStructure("alert_rules", []ColumnDefinition{
		{Name: "id", Type: "integer", NotNull: true, PrimaryKey: true},
		{Name: "host_id", Type: "integer", NotNull: true},
		{Name: "metric_name", Type: "character varying", NotNull: true},
		{Name: "threshold_value", Type: "double precision", NotNull: true},
		{Name: "condition", Type: "character varying", NotNull: true},
...
func verifyTableStructure(tableName string, columns []ColumnDefinition) error {
	for _, expected := range columns {
		...
		// Нормализуем типы данных перед сравнением
		normalizeType := func(t string) string {
			t = strings.ToLower(t)
			t = strings.TrimSpace(t)
			t = strings.ReplaceAll(t, " ", "_")

			// Для строковых типов игнорируем длину
			if strings.HasPrefix(t, "character_varying") {
		...
		actualType := normalizeType(actual.DataType)

		if expectedType != actualType {
			return fmt.Errorf("type mismatch for %s.%s: expected %s, got %s (after normalization: expected %s, got %s)",
				tableName, expected.Name, expected.Type, actual.DataType, expectedType, actualType)
		}
		...
		if expected.Default != "" {
			...
			normalizedActual = strings.Trim(normalizedActual, "'") // Удаляем кавычки
...
func getColumnInfo(tableName, columnName string) (ColumnInfo, error) {
	...
	//log.Printf("Column info: table=%s, column=%s, type=%s, nullable=%s, default=%v",
	//	tableName, columnName, info.DataType, info.IsNullable, info.ColumnDefault)
